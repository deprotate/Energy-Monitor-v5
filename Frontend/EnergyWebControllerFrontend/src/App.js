...
import GreenTechImg from './GreenTechImg.jpg';
import './App.css';
...
function App() {
  ...
  const fetchNumberCounts = async () => {
    try {
      const response = await axios.get('https://backend2-b1j6.onrender.com/api/energy/v1.0/report');
  ...
  return (
    <div className="App">
      ...
      <div>
        <img style={{ width: '300px', height: '500px' }} id="img2" src={GreenTechImg} alt="GreenTechImg" />
      </div>
      <div>
        <img style={{ width: '400px', height: '500px' }} id="img" src={SolarPanel1} alt="SolarPanel1" />
      </div>
      <div id="img3">
        <label >
          Я хз что
